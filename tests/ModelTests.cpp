...
void registry(const fs::path& root){
    ...
    require(qwen.context==8192&&qwen.maxContext==262144&&!qwen.mtpSupported&&qwen.mtpModelPath.empty(),"catalog context capability");
    ...
    require(manager.load(error),error);require(manager.selectedProfile()->model==qwen.model,"selection survives restart");
