...
namespace {
using namespace lvk;
constexpr int kProfile=100,kStart=101,kStop=102,kRestart=103,kWeb=104,kWorkspace=105,kModels=106,kSandbox=107,kUpdate=108,kLog=109,kModelSettings=110;
...
config::Profile* mutableSelectedProfile(){
    ...
    for(auto& profile:settings.profiles)if(profile.name==settings.selectedProfile)return &profile;
...
void openModelSettings(HWND window){
    ...
    std::string error;
    if(ui::showModelSettings(window,*profile,error)){
        if(!gConfig->save(error)){
            MessageBoxA(window,error.c_str(),"Configuration error",MB_OK|MB_ICONERROR);
            return;
        }
        queueLog("Model settings saved for profile: "+profile->name+(gLlama->running()?". Restart AI to apply changes.":"."));
...
LRESULT CALLBACK windowProc(HWND window,UINT message,WPARAM wParam,LPARAM lParam){
    ...
    case WM_CREATE:{
        ...
        for(const auto& button:{std::pair{kStart,L"Start AI"},std::pair{kStop,L"Stop"},std::pair{kRestart,L"Restart"},std::pair{kWeb,L"Open Web UI"},std::pair{kModelSettings,L"Model Settings"},std::pair{kWorkspace,L"Open Workspace"},std::pair{kModels,L"Open Model Folder"},std::pair{kSandbox,L"Rebuild Sandbox"},std::pair{kUpdate,L"Check Updates"}})
        ...
        for(const auto& p:gConfig->settings().profiles){const auto name=wide(p.name);SendMessageW(gProfile,CB_ADDSTRING,0,reinterpret_cast<LPARAM>(name.c_str()));}
        const auto selected=wide(gConfig->settings().selectedProfile);SendMessageW(gProfile,CB_SELECTSTRING,static_cast<WPARAM>(-1),reinterpret_cast<LPARAM>(selected.c_str()));
    ...
    case WM_TIMER:if(wParam==kStaticTimer)requestStatic(window);else if(wParam==kRuntimeTimer)requestRuntime(window);else if(wParam==kLogTimer)flushLog();else if(wParam==kMemoryTimer)requestMemory(window);return 0;
    case kStaticResult:{std::unique_ptr<std::vector<dependencies::Item>> result(reinterpret_cast<std::vector<dependencies::Item>*>(lParam));gStaticItems=std::move(*result);renderStatus();return 0;}
    ...
    case kOperationResult:{std::unique_ptr<std::pair<std::string,bool>> result(reinterpret_cast<std::pair<std::string,bool>*>(lParam));gProcessRunning=result->second;requestStatic(window);requestRuntime(window);requestMemory(window);return 0;}
    case WM_COMMAND:
        ...
        case kStart:startServer(window);break;
        case kStop:stopServer(window,false);break;
        case kRestart:stopServer(window,true);break;
        ...
        case kWorkspace:openPath(gConfig->settings().workspace.wstring());break;
        ...
        return 0;
    case WM_DESTROY:KillTimer(window,kStaticTimer);KillTimer(window,kRuntimeTimer);KillTimer(window,kLogTimer);KillTimer(window,kMemoryTimer);PostQuitMessage(0);return 0;
    }
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,PWSTR,int show){
    ...
    std::string error;
    if(!gConfig->load(error))MessageBoxA(nullptr,error.c_str(),"Configuration error",MB_ICONERROR);
    ...
    gBridge->stop();gWorker.reset();return 0;
}
