...
std::vector<Item> checkStatic(const config::Settings& s,const config::Profile* p){
    ...
    result.push_back({"Docker CLI",docker.empty()?"Docker CLI not found":"Found",!docker.empty()});
    ...
    const bool image=!docker.empty()&&succeeds(docker,L"image inspect "+util::quote(util::wide(s.dockerImage)));
...
std::vector<Item> checkRuntime(const config::Settings& s,bool llamaProcessRunning){
    std::vector<Item> result;const auto docker=process::findOnPath(L"docker");
