...
bool migrateDefaultQwenContext(std::string& text, std::string& error, const std::filesystem::path& path) {
    ...
    if (value == std::string::npos || value >= end || text.compare(value, 4, "8192") != 0 ||
        (value + 4 < end && text[value + 4] >= '0' && text[value + 4] <= '9')) return false;
    text.replace(value, 4, "16384");
...
bool ConfigManager::load(std::string& error) {
    ...
    const auto p=s.find("\"profiles\""); const auto a=s.find('[',p); const auto z=s.find(']',a); if(a!=std::string::npos&&z!=std::string::npos){ std::string block=s.substr(a,z-a); std::regex obj("\\{([^}]*)\\}"); for(std::sregex_iterator i(block.begin(),block.end(),obj),e;i!=e;++i){const auto x=i->str(); Profile q; q.name=read(x,"name",{}); q.model=read(x,"model",{}); if(!q.name.empty()&&!q.model.empty()){q.context=number(x,"context",q.context);q.parallel=number(x,"parallel",q.parallel);q.gpuLayers=number(x,"gpu_layers",q.gpuLayers);q.cpuMoe=number(x,"cpu_moe",q.cpuMoe);q.kvK=read(x,"kv_k",q.kvK);q.kvV=read(x,"kv_v",q.kvV);q.flashAttention=flag(x,"flash_attention",q.flashAttention);q.tools=read(x,"tools",q.tools);q.toolsRuntime=read(x,"tools_runtime",q.toolsRuntime);settings_.profiles.push_back(std::move(q));}} }
    if (migrateDefaultQwenContext(s, error, path_)) {
        for (auto& profile : settings_.profiles) if (profile.name == "Qwen3-Coder-30B-A3B" && profile.context == 8192) profile.context = 16384;
