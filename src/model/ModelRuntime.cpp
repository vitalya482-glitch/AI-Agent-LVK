...
namespace lvk::model {
struct ModelRuntime::Impl { llama_model* model = nullptr; llama_context* context = nullptr; ModelConfig config; std::string path; std::string description; bool generationActive = false; };
...
ModelResult ModelRuntime::configure(ModelConfig config) {
    if (config.contextSize < 128 || config.threadCount < 0 || config.gpuLayers < 0 || config.batchSize < 1 || config.flashAttention < -1 || config.flashAttention > 1) return {false, "Invalid model configuration."};
    if (config.gpuLayers > 0 && !llama_supports_gpu_offload()) return {false, "This build has no GPU backend."};
    if (config.kvCacheOnGpu && !llama_supports_gpu_offload()) return {false, "KV cache on GPU requires a GPU-enabled build."};
    if (config.useMlock && !llama_supports_mlock()) return {false, "This platform does not support locking model memory."};
    const std::string loadedPath = impl_->path;
    const bool reload = impl_->model != nullptr;
    if (reload) unload();
    impl_->config = config;
    if (!reload) return {true, "Model configuration updated."};
    const auto result = load(loadedPath);
    return result.ok ? ModelResult{true, "Configuration applied and model reloaded."} : result;
}
ModelResult ModelRuntime::load(const std::string& path) {
    ...
    params.n_gpu_layers = impl_->config.gpuLayers;
    if (impl_->config.useMmap && impl_->config.useMlock) params.load_mode = LLAMA_LOAD_MODE_MMAP_MLOCK;
    else if (impl_->config.useMmap) params.load_mode = LLAMA_LOAD_MODE_MMAP;
    else if (impl_->config.useMlock) params.load_mode = LLAMA_LOAD_MODE_MLOCK;
    else params.load_mode = LLAMA_LOAD_MODE_NONE;
    ...
    auto context = llama_context_default_params(); context.n_ctx = static_cast<uint32_t>(impl_->config.contextSize); context.n_batch = std::min<uint32_t>(context.n_ctx, static_cast<uint32_t>(impl_->config.batchSize)); context.n_ubatch = context.n_batch; context.n_threads = threads(impl_->config.threadCount); context.n_threads_batch = context.n_threads; context.offload_kqv = impl_->config.kvCacheOnGpu; context.flash_attn_type = static_cast<llama_flash_attn_type>(impl_->config.flashAttention);
...
ModelStatus ModelRuntime::status() const {
    ...
    result.generationActive = impl_->generationActive;
...
ModelResult ModelRuntime::chat(const std::string& message, std::string& response, int maxTokens) {
    response.clear(); if (!impl_->context || message.empty()) return {false, impl_->context ? "Message is empty." : "No model is loaded."}; impl_->generationActive = true;
    struct GenerationGuard { bool& active; ~GenerationGuard() { active = false; } } guard{impl_->generationActive};
